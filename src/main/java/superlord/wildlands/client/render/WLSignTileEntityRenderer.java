...
@OnlyIn(Dist.CLIENT)
public class WLSignTileEntityRenderer extends TileEntityRenderer<WLSignTileEntity>
{
	private final SignTileEntityRenderer.SignModel model = new SignTileEntityRenderer.SignModel();

	public WLSignTileEntityRenderer(TileEntityRendererDispatcher rendererDispatcherIn)
	{
		super(rendererDispatcherIn);
	}

	@Override
	public void render(WLSignTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn)
	{
		...
		RenderMaterial rendermaterial = WLSignTileEntityRenderer.getMaterial(blockstate.getBlock());
	...
	public static RenderMaterial getMaterial(Block blockIn)
	{
		if (blockIn instanceof WLStandingSignBlock || blockIn instanceof WLWallSignBlock)
			return WLSignTileEntityRenderer.getSignMaterial(((AbstractSignBlock) blockIn).getWoodType());
		else
	...
	{
		return new RenderMaterial(Atlases.SIGN_ATLAS, new ResourceLocation(WildLands.MOD_ID, "entity/signs/" + woodtype.getName()));
	}
}
