...
public class WLChestBoatItem extends Item {
	...
	public WLChestBoatItem(WLBoatTypes type, Item.Properties properties) {
		super(properties);
		this.type = type;
	}
	   ...
	   public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand) {
		...
		if (raytraceresult.getType() == HitResult.Type.MISS) {
			...
			if (raytraceresult.getType() == HitResult.Type.BLOCK) {
				WLChestBoat WLChestBoat = new WLChestBoat(world, raytraceresult.getLocation().x, raytraceresult.getLocation().y, raytraceresult.getLocation().z);
				WildLands.LOGGER.info("BOAT ENTITY: " + WLChestBoat.getEntityData());
				WLChestBoat.setWLChestBoatType(this.type);
				WLChestBoat.yRotO = player.yRotO;
				if (!world.noCollision(WLChestBoat, WLChestBoat.getBoundingBox().inflate(-0.1D))) {
					return InteractionResultHolder.fail(itemstack);
				} else {
					if (!world.isClientSide) {
						world.addFreshEntity(WLChestBoat);
