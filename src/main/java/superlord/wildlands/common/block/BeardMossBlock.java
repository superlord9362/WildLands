...
public class BeardMossBlock extends Block implements net.minecraftforge.common.IForgeShearable {
	...
	public BeardMossBlock(AbstractBlock.Properties properties) {
		...
		this.stateToShapeMap = ImmutableMap.copyOf(this.stateContainer.getValidStates().stream().collect(Collectors.toMap(Function.identity(), BeardMossBlock::getShapeForState)));
	}
	...
	private boolean getBlocksAttachedTo(BlockState state) {
		return this.countBlocksBeardMossIsAttachedTo(state) > 0;
	}

	private int countBlocksBeardMossIsAttachedTo(BlockState state) {
		int i = 0;
		...
		if (item == Items.BONE_MEAL) {
			...
			if (chance == 1) {
				...
				if (direction.getAxis().isHorizontal() && !state.get(getPropertyFor(direction))) {
					if (this.hasBeardMossBelow(worldIn, pos)) {
	...
	private boolean hasBeardMossBelow(IBlockReader blockReader, BlockPos pos) {
		int i = 4;
	...
	public boolean isReplaceable(BlockState state, BlockItemUseContext useContext) {
		...
		if (blockstate.isIn(this)) {
			return this.countBlocksBeardMossIsAttachedTo(blockstate) < FACING_TO_PROPERTY_MAP.size();
		} else {
