...
public abstract class WLAbstractTreeFeature<TFC extends WLTreeConfig> extends Feature<TFC> {
	...
	public WLAbstractTreeFeature(Codec<TFC> configCodec) {
		super(configCodec);
	}
	...
	public void placeTrunk(WLTreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
		if (canLogPlaceHere(reader, pos)) {
			this.setFinalBlockState(blockSet, reader, pos, config.getTrunkProvider().getBlockState(random, pos), boundingBox);
		}
	}

	public void placeBranch(WLTreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
		if (canLogPlaceHere(reader, pos)) {
			this.setFinalBlockState(blockSet, reader, pos, config.getTrunkProvider().getBlockState(random, pos), boundingBox);
		}
	}
	...
	public void placeLeaves(WLTreeConfig config, Random random, ISeedReader reader, int x, int y, int z, MutableBoundingBox boundingBox, Set<BlockPos> blockPos) {
	...
	public void placeNetherTrunk(WLTreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
		if (canLogPlaceHereNether(reader, pos)) {
			this.setFinalBlockState(blockSet, reader, pos, config.getTrunkProvider().getBlockState(random, pos), boundingBox);
		}
	}

	public void placeNetherBranch(WLTreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
		if (canLogPlaceHereNether(reader, pos)) {
			this.setFinalBlockState(blockSet, reader, pos, config.getTrunkProvider().getBlockState(random, pos), boundingBox);
		}
	}
	 ...
	 */
	public static boolean isDesiredGroundwDirtTag(IWorldGenerationBaseReader reader, BlockPos pos, WLTreeConfig config) {
	...
	public static boolean isDesiredGroundwNetherTags(IWorldGenerationBaseReader reader, BlockPos pos, WLTreeConfig config) {
		if (config.isPlacementForced())
			return true;
	...
	public static boolean isDesiredGroundwEndTags(IWorldGenerationBaseReader reader, BlockPos pos, WLTreeConfig config) {
		if (config.isPlacementForced())
			return true;
	...
	public static boolean isDesiredGroundwSandTag(IWorldGenerationBaseReader reader, BlockPos pos, WLTreeConfig config) {
		if (config.isPlacementForced())
			return true;
	...
	public void buildTrunkBase(BlockPos centerPos, Set<BlockPos> treeBlocksSet, ISeedReader reader, WLTreeConfig config, Random rand, MutableBoundingBox boundingBox, BlockPos... trunkPositions) {
		if (config.isPlacementForced())
			return;
	 ...
	 */
	public void setSoil(Set<BlockPos> treeBlocksSet, IWorldGenerationBaseReader reader, WLTreeConfig config, Random rand, MutableBoundingBox boundingBox, BlockPos... trunkPositions) {
		if (trunkPositions.length > 0) {
			BlockPos.Mutable mutableTrunk = new BlockPos.Mutable();
			for (BlockPos trunkPos : trunkPositions) {
				mutableTrunk.setPos(trunkPos);
	...
	@SuppressWarnings("deprecation")
	public void setDisk(ISeedReader world, Random random, BlockPos pos, WLTreeConfig config) {
	...
	public boolean placeTree(ISeedReader worldIn, Random rand, BlockPos pos, TFC config) {
		...
		if (!config.decorators.isEmpty()) {
			...
			List<BlockPos> list1 = Lists.newArrayList(set3);
			...
			config.decorators.forEach((p_236405_6_) -> {
				p_236405_6_.func_225576_a_(worldIn, rand, list, list1, set4, mutableboundingbox);
			});
		}
