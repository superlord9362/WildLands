...
public class Coconut10 extends WLAbstractTreeFeature<WLTreeConfig> {

    public Coconut10(Codec<WLTreeConfig> configIn) {
        super(configIn);
    }
    ...
    protected boolean generate(Set<BlockPos> changedBlocks, WorldGenLevel world, Random rand, BlockPos pos, BoundingBox boundsIn, boolean isSapling, WLTreeConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < world.getHeight()) {
            if (!isDesiredGroundwSandTag(world, pos.below(), config)) {
				...
				placeExtraEast(pos, config, rand, changedBlocks, world, mainmutable.set(pos).move(2, 6, 0), boundsIn);
				placeExtraSouth(pos, config, rand, changedBlocks, world, mainmutable.set(pos).move(1, 7, 1), boundsIn);
