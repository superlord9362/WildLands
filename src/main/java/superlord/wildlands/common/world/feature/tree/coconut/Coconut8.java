...
public class Coconut8 extends WLAbstractTreeFeature<WLTreeConfig> {

    public Coconut8(Codec<WLTreeConfig> configIn) {
        super(configIn);
    }
    ...
    protected boolean generate(Set<BlockPos> changedBlocks, WorldGenLevel world, Random rand, BlockPos pos, BoundingBox boundsIn, boolean isSapling, WLTreeConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < world.getHeight()) {
            if (!isDesiredGroundwSandTag(world, pos.below(), config)) {
				...
				placeExtraEast(pos, config, rand, changedBlocks, world, mainmutable.set(pos).move(1, 11, 0), boundsIn);
				placeExtraWest(pos, config, rand, changedBlocks, world, mainmutable.set(pos).move(-1, 10, 0), boundsIn);
				placeExtraSouth(pos, config, rand, changedBlocks, world, mainmutable.set(pos).move(0, 9, 1), boundsIn);
