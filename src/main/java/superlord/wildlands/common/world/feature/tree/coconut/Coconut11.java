...
public class Coconut11 extends WLAbstractTreeFeature<WLTreeConfig> {
    ...
    public Coconut11(Codec<WLTreeConfig> configIn) {
        super(configIn);
    }
    ...
    protected boolean generate(Set<BlockPos> changedBlocks, WorldGenLevel world, Random rand, BlockPos pos, BoundingBox boundsIn, boolean isSapling, WLTreeConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < world.getHeight()) {
            if (!isDesiredGroundwSandTag(world, pos.below(), config)) {
				...
				placeExtraEast(pos, config, rand, changedBlocks, world, mainmutable.set(pos).move(0, 7, 0), boundsIn);
				placeExtraWest(pos, config, rand, changedBlocks, world, mainmutable.set(pos).move(-2, 6, 0), boundsIn);
